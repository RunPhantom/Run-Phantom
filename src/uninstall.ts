...
import {
  disableRunPhantomStartup,
  type RunPhantomStartupDisableOptions,
} from "./runphantom-startup";
...
export interface RunUninstallOptions {
  ...
  runCommand?: RunPhantomStartupDisableOptions["runCommand"];
...
const INSTALLER_BLOCK_START = "# Added by Run Phantom installer";
const INSTALLER_BLOCK_END = "# End Run Phantom installer";
...
function printHelp(): void {
  console.log(`runphantom uninstall ${VERSION} — remove Run Phantom from this machine

USAGE
    runphantom uninstall [--wipe] [--yes] [--dry-run]

WHAT IT DOES
    Removes Run Phantom skills, MCP entries, startup registration, PATH edits,
    and the installed binary. Local traces are preserved by default.

FLAGS
    --wipe       Also delete the local Run Phantom DB, logs, bundles, and caches.
...
function stateDir(homeDir: string): string {
  return path.join(homeDir, ".runphantom");
}
...
function pidPath(homeDir: string): string {
  return path.join(stateDir(homeDir), "runphantom.pid");
}

function portPath(homeDir: string): string {
  return path.join(stateDir(homeDir), "runphantom.port");
}
...
async function stopRunPhantomDaemon(opts: {
  homeDir: string;
  dryRun: boolean;
  removed: string[];
  failures: string[];
  ...
  if (!pid) {
    removeFile(portPath(opts.homeDir), "stale Run Phantom port file", opts);
    return;
  }

  if (opts.dryRun) {
    opts.removed.push(`would stop Run Phantom daemon pid ${pid}`);
    return;
  }

  if (!processAlive(pid)) {
    removeFile(pidPath(opts.homeDir), "stale Run Phantom pid file", opts);
    removeFile(portPath(opts.homeDir), "stale Run Phantom port file", opts);
    return;
  }
  ...
  try {
    ...
    opts.failures.push(`failed to signal Run Phantom pid ${pid}: ${(err as Error).message}`);
    return;
  }
  ...
  while (Date.now() < deadline) {
    if (!processAlive(pid)) {
      removeFile(pidPath(opts.homeDir), "Run Phantom pid file", opts);
      removeFile(portPath(opts.homeDir), "Run Phantom port file", opts);
      opts.removed.push(`stopped Run Phantom daemon pid ${pid}`);
      return;
    }
    await sleep(100);
  }
  opts.failures.push(`Run Phantom pid ${pid} did not exit within ${opts.timeoutMs}ms`);
}
...
function removeEmptyDir(target: string): void {
  try {
    ...
    // Directory absence and non-empty directories are both safe no-ops here.
  }
}
...
function fallbackEntries(homeDir: string, cwd: string): InstallRegistryEntry[] {
  ...
  // Windsurf's global config needs an explicit fallback when discovery data changes.
...
function removeStartup(opts: RunUninstallOptions & { removed: string[]; failures: string[] }): void {
  if (opts.dryRun) {
    opts.removed.push("would remove Run Phantom startup registration");
    return;
  }
  const result = disableRunPhantomStartup({
...
function wipeState(
  ...
  const db = path.join(root, "runphantom.db");
  const targets = [
    db,
    `${db}-wal`,
    `${db}-shm`,
    path.join(root, "runphantom.log"),
    path.join(root, "runphantom.startup.log"),
    pidPath(homeDir),
    portPath(homeDir),
  ...
  for (const target of targets) removePath(target, "Run Phantom state", opts);

  // Replay command logs use the project basename in the filename, so glob.
  try {
    for (const entry of fs.readdirSync(root)) {
      if (/^replay-.*\.log$/.test(entry)) {
        removePath(path.join(root, entry), "Run Phantom state", opts);
      }
    }
  } catch {
...
function isSafeBinaryPath(execPath: string, homeDir: string): boolean {
  ...
  if (base !== "runphantom" && base !== "runphantom.exe") return false;
  if (execPath.includes(`${path.sep}Cellar${path.sep}`)) return false;
  const expected = path.join(homeDir, ".runphantom", "bin", path.basename(execPath));
...
function removeBinary(
  ...
  removeFile(execPath, "Run Phantom binary", opts);
  removeFile(`${execPath}.prev`, "previous Run Phantom binary", opts);
}

function cleanupEmptyStateDirs(homeDir: string): void {
  removeEmptyDir(path.join(homeDir, ".runphantom", "bin"));
  removeEmptyDir(path.join(homeDir, ".runphantom"));
}
...
export async function runUninstall(opts: RunUninstallOptions = {}): Promise<RunUninstallResult> {
  ...
  if (wipe && !daemonStopFailed) {
    ...
    warnings.push("skipped local state wipe because the Run Phantom daemon could not be stopped");
  }
...
async function confirmUninstall(wipe: boolean): Promise<boolean> {
  const detail = wipe
    ? "This will remove Run Phantom integrations, the binary, and all local trace data."
    : "This will remove Run Phantom integrations and the binary. Local trace data will be preserved.";
...
export async function cmdUninstall(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (err instanceof UsageError) {
      ...
      console.error("run `runphantom uninstall --help` for usage.");
      return 64;
    }
    throw err;
  }
