/** Secret redaction for captured evidence.
 *
 * Runs before anything is persisted or transported. Values matching sensitive
 * key names are replaced wholesale rather than masked, and URLs are rewritten
 * so a credential in a query string cannot survive into a stored report.
 */
...
// legitimate inspect/state output.
 ...
 * eventually ship an app that leaks, and the runtime verification module would be the thing that recorded it.
 */
...
export function buildRedactionPolicy(
  ...
  if (exemptedCredentials.length > 0 && onWarn !== undefined) {
    onWarn(
      `[Run Phantom] redact.allow is exempting ${exemptedCredentials.join(', ')} from redaction. The ` +
...
/** Decode names only for matching; keep their original spelling in the returned URL. */
function decodedUrlName(name: string): string {
  try {
    return decodeURIComponent(name);
  } catch {
    // Malformed escapes are observable input, not a reason to interrupt the host app.
    return name;
  }
}
...
export function redactUrl(
  ...
  for (let i = 0; i + 1 < segments.length; i++) {
    ...
    if (
      ...
      SENSITIVE_PATH_SEGMENT.test(decodedUrlName(name))
    ) {
  ...
  if (hash.length > 1) {
    newHash = hash.replace(
      ...
      (m: string, delimiter: string, key: string) =>
        isSensitive(decodedUrlName(key)) ? `${delimiter}${key}=${REDACTED_VALUE}` : m,
    );
