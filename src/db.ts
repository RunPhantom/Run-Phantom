...
import { SOURCE_MIGRATIONS_DIR } from "./source-paths";
import { IS_PACKAGED_RUNTIME, VERSION } from "./version";

const RUNPHANTOM_DB_PATH_ENV_VAR = "RUNPHANTOM_DB_PATH";

function resolveDbPath(): string {
  const explicit = process.env[RUNPHANTOM_DB_PATH_ENV_VAR];
  if (explicit && explicit.trim()) {
    return explicit;
  }
  return path.join(os.homedir(), ".runphantom", "runphantom.db");
}

function resolveMigrationsFolder(): string {
  if (
    !IS_PACKAGED_RUNTIME &&
    fs.existsSync(path.join(SOURCE_MIGRATIONS_DIR, "meta", "_journal.json"))
  ) {
...
function ensureEmbeddedMigrationsFolder(): string {
  const cacheRoot = path.join(os.homedir(), ".runphantom", "migrations", VERSION);
...
type RunPhantomDb = BunSQLiteDatabase<typeof schema> & { $client: Database };
let _drizzleDb: RunPhantomDb | null = null;
...
export function getDrizzleDb(): RunPhantomDb {
  ...
  try {
    // Both pragmas are required for the table rebuild in
    // 0003_span_identity_per_run.sql, which creates `__new_spans`, drops
    // `spans`, then renames. They must be set on the connection: drizzle runs
    // each migration inside a transaction, and SQLite ignores PRAGMA there, so
    // the copies inside the .sql file never take effect.
    //
    // legacy_alter_table: the `runs_with_hints` view from 0001 selects from
    // `spans`. Dropping the table leaves that view dangling, and since SQLite
    // 3.25 an ALTER TABLE ... RENAME reparses every view and trigger to fix up
    // references — so it aborts on the broken view with
    // "Failed to run the query 'ALTER TABLE `__new_spans` RENAME TO `spans`'".
    // Legacy mode skips that reparse, which is correct here because the rename
    // is what makes the view valid again. macOS links an older system SQLite
    // that did not reparse, so this only ever failed on Linux.
    //
    // foreign_keys: `spans.run_id` references `runs.id`, and SQLite requires
    // enforcement off while a referenced table is dropped and recreated.
    _sqliteDb.exec("PRAGMA legacy_alter_table = ON");
    _sqliteDb.exec("PRAGMA foreign_keys = OFF");
    migrate(_drizzleDb, { migrationsFolder: resolveMigrationsFolder() });
    _sqliteDb.exec("PRAGMA foreign_keys = ON");
    _sqliteDb.exec("PRAGMA legacy_alter_table = OFF");
  } catch (err) {
    closeDb();
    throw new Error(
      `Failed to migrate Run Phantom DB at ${dbPath}: ${(err as Error).message}\n` +
        `If you do not need the local trace history, run: runphantom reset`,
    );
  }
  return _drizzleDb;
}

type DrizzleExecutor = Pick<RunPhantomDb, "run" | "get" | "all" | "values">;
...
function assertTraceSafeTables(tokens: readonly SqlToken[]): void {
  ...
  const validateSource = (index: number): void => {
    ...
    if (!TRACE_SAFE_TABLES.has(source.value)) {
      throw new Error(`query_traces may read only trace-safe tables; rejected ${source.value}`);
    }
    const next = tokens[index + 1];
    if (next?.kind === "punctuation" && (next.value === "." || next.value === "(")) {
      throw new Error("query_traces may read only bare trace-safe tables");
    }
  };
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (token.kind === "identifier" && !token.quoted && token.value === "from") {
      fromClauses.add(depth);
      validateSource(index + 1);
      continue;
    }
    if (token.kind === "identifier" && !token.quoted && token.value === "join") {
      validateSource(index + 1);
      continue;
    }
...
/**
 * Blanks the CONTENTS of string literals and quoted identifiers, keeping the
 * quotes and overall length.
 *
 * The guards below are regexes, and they were run over the raw SQL — so a
 * perfectly legal `WHERE name LIKE '%delete%'` tripped the blocked-keyword rule
 * and `SELECT 'a;b'` tripped the one-statement rule. Only the code outside
 * quotes can carry a keyword or a statement separator, so mask the rest first.
 */
  ...
  for (let i = 0; i < sql.length;) {
    ...
    if (char === "'" || char === '"' || char === "`" || char === "[") {
      ...
      while (i < sql.length) {
        ...
        if (sql[i] === close) { out += close; i++; break; }
        out += " ";
        i++;
      }
      continue;
...
export function queryTraces(sql: string, opts: QueryTracesOpts = {}): QueryTracesResult {
  ...
  // Floor before clamping: these are bound as SQL integers, and a fractional
  // limit reached SQLite as a bare "datatype mismatch" rather than a usable
  // message about the caller's own argument.
  const asInt = (v: unknown, fallback: number) =>
    typeof v === "number" && Number.isFinite(v) ? Math.floor(v) : fallback;
  const limit = Math.max(1, Math.min(1000, asInt(opts.limit, 100)));
  const maxBytes = Math.max(1_000, Math.min(1_000_000, asInt(opts.maxBytes, 120_000)));
...
/** Wall-clock budget for one trace query before the daemon stops waiting on it. */
export const TRACE_QUERY_TIMEOUT_MS = 5_000;

/**
 * Run a trace query with a hard time bound.
 *
 * Same validation and byte-budget trimming as queryTraces, but the SQL executes in
 * a worker (see query-worker.ts) so exceeding the budget returns an error to the
 * caller instead of blocking the daemon's single thread until SQLite finishes.
 */
export async function queryTracesBounded(
  sql: string,
  opts: QueryTracesOpts = {},
  timeoutMs: number = TRACE_QUERY_TIMEOUT_MS,
  ...
  try {
    const rawRows = await new Promise<Record<string, unknown>[]>((resolve, reject) => {
      timer = setTimeout(() => {
        reject(new Error(`query exceeded the ${timeoutMs}ms limit — narrow it with a WHERE clause or a smaller join`));
      }, timeoutMs);
      worker.onmessage = (event: MessageEvent<{ ok: boolean; rows?: Record<string, unknown>[]; error?: string }>) => {
        if (event.data.ok) resolve(event.data.rows ?? []);
        else reject(new Error(event.data.error ?? "query failed"));
      };
      worker.onerror = (event: ErrorEvent) => reject(new Error(event.message || "query worker failed"));
      worker.postMessage({ dbPath, sql: safeSql, limit: limit + 1 });
    });
    ...
    return { columns, rows: keptRows, row_count: keptRows.length, truncated, elapsed_ms: Date.now() - started };
  } finally {
    if (timer) clearTimeout(timer);
    // Terminate unconditionally: on timeout this is what frees the daemon, and on
    // success the worker has already done its only job.
    worker.terminate();
  }
}
...
export function closeDb(): void {
  const hadConnection = _sqliteDb !== null;
  _drizzleDb = null;
  if (_sqliteDb) {
    _sqliteDb.close();
    _sqliteDb = null;
  ...
  if (hadConnection && process.platform === "win32") Bun.gc(true);
}
...
/**
 * Last-resort stitcher for a replay whose agent never echoed its replayRunId.
 *
 * It must only ever match a run the AGENT produced. Every concurrent replay of
 * the same source writes a placeholder under the identical `replay:<name>` event
 * name, so matching on event name alone let one replay adopt another's
 * placeholder, overwrite its metadata and name, and hand back the wrong run id —
 * corrupting both. Two conditions rule that out: a placeholder carries a
 * `"replay":` metadata block (a real OTLP run only gets one after promotion),
 * and it has no spans, whereas the run being looked for is defined by having
 * shipped some.
 */
export function findRecentRunByEventName(eventName: string, afterTimestamp: number, excludeId?: string): any {
  const conditions = [
    eq(schema.runs.event_name, eventName),
    gte(schema.runs.started_at, afterTimestamp),
    drizzleSql`(${schema.runs.metadata} IS NULL OR ${schema.runs.metadata} NOT LIKE '%"replay":%')`,
    drizzleSql`EXISTS (SELECT 1 FROM spans WHERE spans.run_id = ${schema.runs.id})`,
  ];
  if (excludeId) conditions.push(ne(schema.runs.id, excludeId));
  return getDrizzleDb()
    .select()
    .from(schema.runs)
...
export function deleteRun(runId: string) {
  getDrizzleDb().transaction((tx) => {
    ...
    // Annotations are not FK-cascaded, so leaving them behind orphans every note
    // and flag on the deleted run: they stay countable in the annotations API and
    // reattach to whatever later run reuses the id.
    tx.delete(schema.annotations).where(eq(schema.annotations.run_id, runId)).run();
    tx.delete(schema.runs).where(eq(schema.runs.id, runId)).run();
  });
}

/**
 * Moves a placeholder run's annotations onto the run that replaced it.
 *
 * A replay creates a placeholder run immediately so the URL resolves, then
 * deletes it once the agent's real OTLP trace arrives. Anything the user
 * annotated on the placeholder while the replay was in flight belongs to the
 * promoted run, not to a row that is about to disappear. span_id is cleared
 * because placeholder spans do not exist under the new run id.
 */
/**
 * Clears one run's spans so an import can restore the file exactly.
 *
 * Import used to upsert on top of whatever was already stored, so re-importing
 * a run that had since gained spans produced a union of the two — a run that
 * matched neither the file nor the original. Annotations are deliberately kept:
 * they are the user's own work, not part of the imported trace.
 */
export function deleteRunSpans(runId: string): void {
  getDrizzleDb().transaction((tx) => {
    tx.delete(schema.spans).where(eq(schema.spans.run_id, runId)).run();
    tx.delete(schema.live_events).where(eq(schema.live_events.trace_id, runId)).run();
  });
...
export function reassignAnnotations(fromRunId: string, toRunId: string): number {
  if (fromRunId === toRunId) return 0;
  const rows = getDrizzleDb()
    .update(schema.annotations)
    .set({ run_id: toRunId, span_id: null })
    .where(eq(schema.annotations.run_id, fromRunId))
...
export function insertSpan(span: { id: string; run_id: string; parent_span_id?: string; name: string; span_type?: string; status?: string; input_payload?: string; output_payload?: string; start_time_ms: number; end_time_ms: number; duration_ms: number; model?: string; provider?: string; input_tokens?: number; output_tokens?: number; attributes?: string }) {
  ...
  getDrizzleDb()
    .insert(schema.spans)
    .values(row)
    // Identity is (run_id, id): OTel only guarantees a SpanId is unique within its
    // trace, so two traces may legitimately share one. Conflicting on the composite
    // key keeps re-ingest idempotent per run while letting both runs keep their span.
    .onConflictDoUpdate({ target: [schema.spans.run_id, schema.spans.id], set: row })
    .run();
}

/**
 * Run a batch of writes as one unit.
 *
 * OTLP ingest writes a run row and then its spans. Without a transaction a batch
 * that fails partway leaves the runs it already committed plus an orphan run row
 * for the trace that failed, and still answers 500 — so the exporter retries and
 * duplicates the successful half. bun:sqlite is synchronous and these helpers all
 * share one connection, so plain nested calls join this transaction.
 */
export function runInTransaction<T>(fn: () => T): T {
  return getDrizzleDb().transaction(() => fn()) as T;
}
...
export function upsertEventSpan(span: { id: string; run_id: string; name: string; span_type?: string; status?: string; input_payload?: string; output_payload?: string; start_time_ms: number; end_time_ms: number; duration_ms: number; model?: string; attributes?: string }) {
  getDrizzleDb()
    ...
    .onConflictDoUpdate({
      // Must match the composite primary key; a bare `id` target names no unique
      // index and SQLite rejects the whole statement at runtime.
      target: [schema.spans.run_id, schema.spans.id],
      set: {
...
export function getRuns(limit = 200) {
  // Deliberately not `select()`: runs_with_hints also derives payload_total_chars,
  // which sums LENGTH() over every payload of every run it returns. Measured on
  // 400 runs x 40 spans, that one column was 194ms of a 232ms query while the
  // rest of the row cost 1ms — and the sidebar never renders it. It stays on the
  // view for query_traces discovery; the list just stops asking for it.
  const rows = getDrizzleDb()
    .select({
      id: schema.runs_with_hints.id,
      event_id: schema.runs_with_hints.event_id,
      name: schema.runs_with_hints.name,
      event_name: schema.runs_with_hints.event_name,
      display_name: schema.runs_with_hints.display_name,
      user_id: schema.runs_with_hints.user_id,
      convo_id: schema.runs_with_hints.convo_id,
      started_at: schema.runs_with_hints.started_at,
      last_updated_at: schema.runs_with_hints.last_updated_at,
      metadata: schema.runs_with_hints.metadata,
      model: schema.runs_with_hints.model,
      finished: schema.runs_with_hints.finished,
      span_count: schema.runs_with_hints.span_count,
      live_event_count: schema.runs_with_hints.live_event_count,
    })
    .from(schema.runs_with_hints)
    .orderBy(desc(schema.runs_with_hints.last_updated_at))
    .limit(limit)
    .all();
  if (rows.length === 0) return rows.map((r) => ({ ...r, error_count: 0 }));
  // The list view needs a failed/complete signal so a run with errors reads
  // "Run failed" in the sidebar, matching the detail header. The runs_with_hints
  // view intentionally stays lean, so derive error counts in one scoped query.
  const ids = rows.map((r) => r.id).filter((id): id is string => id != null);
  if (ids.length === 0) return rows.map((r) => ({ ...r, error_count: 0 }));
  const errCounts = getDrizzleDb()
    .select({ run_id: schema.spans.run_id, n: count() })
    .from(schema.spans)
    .where(and(eq(schema.spans.status, "ERROR"), inArray(schema.spans.run_id, ids)))
    .groupBy(schema.spans.run_id)
    .all();
  const errByRun = new Map(errCounts.map((e) => [e.run_id, e.n]));
  return rows.map((r) => ({ ...r, error_count: r.id != null ? errByRun.get(r.id) ?? 0 : 0 }));
}
...
/** See getSpanMeta on why an unscoped span id is ambiguous and how it resolves. */
export function getSpanPayloadColumn(spanId: string, target: "input" | "output", runId?: string): string | null {
  ...
  const row = getDrizzleDb()
    ...
    .from(schema.spans)
    .innerJoin(schema.runs, eq(schema.runs.id, schema.spans.run_id))
    .where(runId ? and(eq(schema.spans.id, spanId), eq(schema.spans.run_id, runId)) : eq(schema.spans.id, spanId))
    .orderBy(desc(schema.runs.last_updated_at))
    .limit(1)
    .get();
  if (!row) return null;
  return row.payload ?? "";
...
/** See getSpanMeta on why an unscoped span id is ambiguous and how it resolves. */
export function getSpanById(spanId: string, runId?: string) {
  const row = getDrizzleDb()
    .select()
    .from(schema.spans)
    .innerJoin(schema.runs, eq(schema.runs.id, schema.spans.run_id))
    ...
    .limit(1)
    .get() as any;
  return row ? attachNormalized(row.spans ?? row) : null;
}
...
/**
 * Look up a span by id, optionally scoped to its run.
 *
 * Span identity is (run_id, id): OTel only guarantees a SpanId is unique within
 * its trace, so an unscoped lookup is genuinely ambiguous. Callers that know the
 * run should pass it; the unscoped path resolves to the most recently updated run
 * so the result is at least deterministic and stable across calls.
 */
export function getSpanMeta(spanId: string, runId?: string): SpanMetaRow | null {
  return getDrizzleDb()
    .select({
      id: schema.spans.id,
...
export function clearAll() {
  getDrizzleDb().transaction((tx) => {
    tx.delete(schema.evaluation_reviews).run();
    tx.delete(schema.evaluation_experiments).run();
    tx.delete(schema.evaluation_revisions).run();
    tx.delete(schema.evaluation_datasets).run();
    tx.delete(schema.verification_reports).run();
    tx.delete(schema.verification_flows).run();
    tx.delete(schema.live_events).run();
    tx.delete(schema.spans).run();
    // Annotations are not FK-cascaded. Left behind they still count in the
    // annotations API and re-attach to whatever run later reuses the id.
...
export function getRunOutline(runId: string, payloadPreviewChars = 80): RunOutline {
  ...
  // Iterative and cycle-guarded. Nothing in OTLP forbids a span from naming a
  // descendant as its parent, and the recursive version recursed forever on such
  // a pair: a two-span cycle overflowed the stack and made the run permanently
  // unviewable through both this endpoint and the MCP outline tool. A span
  // inside a cycle is reported at the depth of the first node that closed it.
  const depthCache = new Map<string, number>();
  function depthOf(start: any): number {
    const chain: any[] = [];
    const seen = new Set<string>();
    let node = start;
    let base = 0;

    while (node?.parent_span_id) {
      const cached = depthCache.get(node.id);
      if (cached !== undefined) { base = cached; break; }
      if (seen.has(node.id)) { base = 0; break; }
      seen.add(node.id);
      chain.push(node);
      const parent = spanById.get(node.parent_span_id);
      if (!parent) { base = 0; break; }
      node = parent;
    }
    if (node && !node.parent_span_id) base = 0;

    for (let i = chain.length - 1; i >= 0; i--) {
      base += 1;
      depthCache.set(chain[i].id, base);
    }
    return depthCache.get(start.id) ?? 0;
  }
...
export function listSpansFiltered(runId: string, opts: ListSpansOpts = {}): SpanSkeleton[] {
  ...
  // SQLite has no REGEXP, so name_regex has to be evaluated in JS. Doing that
  // after LIMIT meant `?name_regex=foo&limit=50` returned however many of the
  // first 50 rows happened to match, not the first 50 matching rows — page 2
  // could hold matches page 1 had silently dropped. Resolve the matching ids
  // first over an id+name scan, then page that.
  let idFilter: string[] | null = null;
  if (f.name_regex) {
    const risk = redosRisk(f.name_regex);
    if (risk) throw new Error(`unsafe name_regex: ${risk}`);
    let re: RegExp;
    try { re = new RegExp(f.name_regex, "i"); }
    catch (err: any) { throw new Error(`invalid name_regex: ${err.message}`); }
    const names = rawQuery(`
      SELECT id, name FROM spans WHERE ${where.join(" AND ")} ORDER BY ${orderBy}
    `).all(...params) as { id: string; name: string }[];
    idFilter = names.filter((r) => re.test(r.name)).map((r) => r.id);
...
/**
 * Rows `listSpansFiltered` would match if it were not paged.
 *
 * That function returns a bare array capped at 500, so a run with more spans
 * came back looking complete: the span tree rendered orphans whose parents were
 * past the cap and nothing said why. The route reports this alongside the page
 * so a caller can tell a full result from a truncated one.
 */
export function countSpansFiltered(runId: string, opts: ListSpansOpts = {}): number {
  const where: string[] = ["run_id = ?"];
  const params: any[] = [runId];
  const f = opts.filter ?? {};
  ...
  if (f.name_regex) {
    if (redosRisk(f.name_regex)) return 0;
    let re: RegExp;
    try { re = new RegExp(f.name_regex, "i"); } catch { return 0; }
    const names = rawQuery(`SELECT name FROM spans WHERE ${where.join(" AND ")}`).all(...params) as { name: string }[];
    return names.filter((r) => re.test(r.name)).length;
  }

  const row = rawQuery(`SELECT COUNT(*) AS c FROM spans WHERE ${where.join(" AND ")}`).get(...params) as { c: number };
  return row?.c ?? 0;
}
...
/**
 * Rejects search patterns that can backtrack exponentially.
 *
 * The pattern comes straight from a query string and is run against every
 * payload in a run, on the daemon's only thread. A star-height-2 pattern like
 * `(a+)+$` against a few dozen KB of non-matching text does not return in any
 * useful time and there is no way to interrupt a regex mid-exec in JS, so the
 * whole daemon is gone until it is restarted.
 *
 * Star height >= 2 — an unbounded quantifier applied to a group that itself
 * contains an unbounded quantifier — is the necessary condition for exponential
 * blowup, so refusing it is sufficient here. Polynomial cases survive but stay
 * bounded by the per-payload match cap and the size of a single payload.
 *
 * Returns a reason string when the pattern is rejected, or null when it is fine.
 */
export function redosRisk(pattern: string): string | null {
  if (pattern.length > 1000) return "pattern exceeds 1000 characters";

  // Depth of the group each unbounded quantifier lives in, so a quantifier on a
  // closing group can ask "did my own body contain one?".
  const openIndices: number[] = [];
  const unboundedInGroup: boolean[] = [false];
  let inClass = false;
  ...
  for (let i = 0; i < pattern.length; i++) {
    ...
    if (c === "(") { openIndices.push(i); unboundedInGroup.push(false); continue; }

    if (c === ")") {
      const bodyHadUnbounded = unboundedInGroup.pop() ?? false;
      openIndices.pop();
      if (unboundedInGroup.length === 0) unboundedInGroup.push(false);
      const q = quantifierAt(pattern, i + 1);
      if (q.unbounded) {
        if (bodyHadUnbounded) return "nested unbounded quantifiers can backtrack exponentially";
        unboundedInGroup[unboundedInGroup.length - 1] = true;
        i += q.length;
      } else if (bodyHadUnbounded) {
        unboundedInGroup[unboundedInGroup.length - 1] = true;
      }
      continue;
    }
...
function quantifierAt(pattern: string, i: number): { unbounded: boolean; length: number } {
  const c = pattern[i];
  if (c === "*" || c === "+") return { unbounded: true, length: 1 };
  if (c === "{") {
    const close = pattern.indexOf("}", i);
    if (close === -1) return { unbounded: false, length: 0 };
    const body = pattern.slice(i + 1, close);
    if (/^\d+,$/.test(body)) return { unbounded: true, length: close - i + 1 };
    return { unbounded: false, length: 0 };
  }
  return { unbounded: false, length: 0 };
}

// Bounds the joined live-event transcript a single search will assemble. A long
// streaming run can hold far more text than any one search needs to scan.
const LIVE_EVENT_SEARCH_CHAR_CAP = 2_000_000;
...
export function searchRun(runId: string, opts: SearchRunOpts): { matches: SearchMatch[]; truncated: boolean } {
  ...
  if (opts.regex) {
    const risk = redosRisk(opts.pattern);
    if (risk) throw new Error(`unsafe regex: ${risk}`);
    let re: RegExp;
  ...
  // Live event scope
  // A span_type filter selects spans by their type, and a live event has none —
  // so the filter was silently dropped for this scope and a narrowed search
  // returned events it had been asked to exclude.
  if (scopes.has("live_event") && !opts.span_type && matches.length < cap) {
    // Searched as one continuous transcript, not event by event.
    //
    // A streamed reply arrives as many small deltas, so a phrase the user can
    // plainly read in the Conversation tab ("brown fox") is usually split across
    // two rows and matched neither of them. The per-row LIKE prefilter had the
    // same blind spot, discarding the rows before matching even ran. Joining
    // first and mapping offsets back gives the search the same view of the text
    // that the reader has.
    const rows = rawQuery(
      `SELECT id, span_id, content FROM live_events WHERE trace_id = ? ORDER BY id ASC`
    ).all(runId) as any[];

    let joined = "";
    const segments: Array<{ start: number; end: number; span_id: string | null }> = [];
    for (const r of rows) {
      const text = r.content ?? "";
      if (!text) continue;
      if (joined.length >= LIVE_EVENT_SEARCH_CHAR_CAP) { truncated = true; break; }
      segments.push({ start: joined.length, end: joined.length + text.length, span_id: r.span_id ?? null });
      joined += text;
    }

    if (joined) {
      for (const [s, e] of matcher(joined)) {
        // Attribute the match to the event its first character falls in.
        const segment = segments.find((seg) => s >= seg.start && s < seg.end);
        matches.push({
          span_id: segment?.span_id ?? null, span_name: null, scope: "live_event",
          match_range: [s, e],
          snippet: buildSnippet(joined, s, e, ctx),
          payload_total_chars: joined.length,
        });
