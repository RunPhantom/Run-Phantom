...
export function listClaudeSessions(cwd: string): ClaudeSessionSummary[] {
  return projectSessionDirs(cwd)
    ...
    // A transcript with no messages has nothing to preview and nothing to
    // resume, so it can only ever be a row that does nothing when clicked.
    .filter((session): session is ClaudeSessionDetail => !!session && session.message_count > 0)
...
export function getLatestClaudeLoadout(cwd: string): ClaudeLoadout | null {
  const candidates = projectSessionDirs(cwd)
    .filter((dir) => fs.existsSync(dir))
    .flatMap((dir) => fs.readdirSync(dir)
      .filter((name) => name.endsWith(".jsonl"))
      .map((name) => path.join(dir, name)))
    .slice(0, 400);
  const files = sortByMtimeDesc(candidates).slice(0, 20);
...
function readClaudeSessionFile(filePath: string): ClaudeSessionDetail | null {
  ...
  for (const line of lines) {
    ...
    if (event.type === "last-prompt" && typeof event.lastPrompt === "string") {
      lastPrompt = stripRunPhantomEnvelope(event.lastPrompt);
      continue;
    }
    ...
    const visibleContent = stripRunPhantomEnvelope(content);
    ...
    if (event.type === "assistant" && claudeMessageId && previous?.id === claudeMessageId) {
      ...
      previous.content = stripRunPhantomEnvelope(blocksText(mergedBlocks)) || previous.content || visibleContent;
...
function stripRunPhantomEnvelope(text: string): string {
  return text
    .replace(/<runphantom_message>[\s\S]*?<\/runphantom_message>\s*/g, "")
    .trim();
}
