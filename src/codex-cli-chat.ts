...
import {
  ...
  defaultAgentLoadout,
  resolveRunPhantomMcpCommand,
  runPhantomSidepanelPrompt,
...
export function runCodexCliChat(
  ...
  const child = spawn(process.env.RUNPHANTOM_CODEX_BIN ?? "codex", buildCodexArgs(input), {
    cwd: input.cwd,
    env: { ...process.env },
...
export function buildCodexArgs(input: CodexCliChatInput): string[] {
  const mcpCommand = resolveRunPhantomMcpCommand();
  const runPhantomEnv = [
    `RUNPHANTOM_URL=${JSON.stringify(input.backendUrl)}`,
    `RUNPHANTOM_AGENT_PROVIDER="codex"`,
    `RUNPHANTOM_ANNOTATION_SOURCE=${JSON.stringify(agentAnnotationSource("codex"))}`,
  ];
  const commonArgs = [
    "-C",
    input.cwd,
    "-c",
    `mcp_servers.runphantom.command=${JSON.stringify(mcpCommand.command)}`,
    "-c",
    `mcp_servers.runphantom.args=${JSON.stringify(mcpCommand.args)}`,
    "-c",
    `mcp_servers.runphantom.env={${runPhantomEnv.join(",")}}`,
  ];
  if (process.env.RUNPHANTOM_CODEX_BYPASS_PERMISSIONS !== "0") {
    commonArgs.unshift("--dangerously-bypass-approvals-and-sandbox");
  } else {
    commonArgs.unshift("-a", process.env.RUNPHANTOM_CODEX_APPROVAL_POLICY ?? "never");
    const sandbox = process.env.RUNPHANTOM_CODEX_SANDBOX;
...
function directReplySystemPrompt(input: CodexCliChatInput): string {
  return runPhantomSidepanelPrompt({
    provider: "codex",
    localMcpName: "runphantom",
    runId: input.runId,
  });
}
...
function userPrompt(input: CodexCliChatInput): string {
  ...
  lines.push("</runphantom_message>", "", input.content);
  return lines.join("\n");
}
