...
export const aiSdkLlmAdapter: SpanAdapter = {
  ...
  apply(input): AdapterMatch | null {
    ...
    if (Array.isArray(parsed)) {
      const sys: string[] = [];
      for (const m of parsed as Array<Record<string, unknown> | null>) {
        // A null or primitive element throws on the property read below, and the
        // throw propagates out of ingest and rejects the whole OTLP batch — one
        // malformed message in one span costing every other trace in the export.
      ...
      if (Array.isArray(p.messages)) {
        for (const m of p.messages as Array<Record<string, unknown> | null>) {
          if (!m || typeof m !== "object") continue;
          const role = (m.role as string | undefined) ?? "unknown";
          const content = extractContent(m.content);
