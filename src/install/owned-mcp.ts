...
export const CANONICAL_MCP_SERVER_NAME = "runphantom";
export const MCP_FINGERPRINT_ARGS = ["mcp"] as const;
...
function commandBasenameLooksOwned(command: unknown): boolean {
  ...
  return base === CANONICAL_MCP_SERVER_NAME;
}
...
function argsEndWithFingerprint(args: unknown): boolean {
  ...
  const entrypoint = args[args.length - 2];
  return typeof entrypoint === "string" && path.basename(entrypoint) === "index.ts"
    && MCP_FINGERPRINT_ARGS.every((expected, i) => tail[i] === expected);
}
...
export interface FindOwnedOptions {
  ...
  cwd?: string;
  homeDir?: string;
  // Setup keeps the freshly written canonical entry; uninstall removes it.
...
function collectOwnedFromConfig(
  ...
  for (const [serverName, rawConfig] of Object.entries(entries)) {
    ...
    const matchedBy = serverName === CANONICAL_MCP_SERVER_NAME
      ? classifyOwnership(rawConfig) ?? "command"
      : classifyOwnership(rawConfig);
