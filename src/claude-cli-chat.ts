...
export function runClaudeCliChat(
  ...
  const child = spawn(process.env.RUNPHANTOM_CLAUDE_BIN ?? "claude", buildClaudeArgs(input), {
    cwd: input.cwd,
    env: { ...process.env },
...
export function buildClaudeArgs(input: ClaudeCliChatInput): string[] {
  ...
  const mcpConfig = buildMcpConfig(input.backendUrl, mcpCommand);
  ...
  if (input.resumeSessionId) {
    ...
    args.push("--name", `Run Phantom ${Date.now().toString(36)}`);
  }
...
export function buildMcpConfig(
  backendUrl: string,
  mcpCommand = resolveRunPhantomMcpCommand(),
): {
...
`;
  return `${process.env.RUNPHANTOM_HOOK_NODE_BIN ?? "node"} -e ${shellArg(script)}`;
}
