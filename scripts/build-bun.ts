#!/usr/bin/env bun
/**
 * Build a single self-contained `runphantom` binary.
 * Outputs: build/bun/runphantom-<target>[.exe].
 *
 ...
 * compiled binary can drop them into IDE config dirs at `runphantom setup`.
...
function outputName(target: Target): string {
  ...
  return `runphantom-${target}${ext}`;
}

function ensureNoCheckoutPath(outfile: string): void {
  ...
  for (const checkoutPath of checkoutPaths) {
    if (checkoutPath && binary.includes(Buffer.from(checkoutPath))) {
      throw new Error(
        `[build-bun] HARD GATE FAILURE: ${outfile} contains the local checkout path`,
      );
    }
  }
  console.log("[build-bun]   ✓ local checkout path absent");
}

/**
 * Resolve the version string we embed into the binary.
 *
 *   - CI sets `RUNPHANTOM_VERSION` from the pushed tag.
 *   - Local builds fall back to package.json's `version` plus a `-local` suffix,
 *     so `runphantom --version` is never blank.
 */
function resolveVersion(): string {
  const fromEnv = process.env.RUNPHANTOM_VERSION;
...
function buildOne(target: Target, version: string): string {
  ...
  run("bun", [
    ...
    "--minify",
    `--target=${target}`,
    `--define=__RUNPHANTOM_VERSION__="${version}"`,
    "--define=__RUNPHANTOM_PACKAGED__=true",
    "--define=__RUNPHANTOM_SOURCE_ROOT__=\"/runphantom\"",
    ENTRY,
    "--outfile",
    outfile,
  ]);
  ensureNoCheckoutPath(outfile);
