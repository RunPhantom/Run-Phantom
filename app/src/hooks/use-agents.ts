...
/**
 * Live view of `~/.runphantom/agents.json` and per-agent `/health` status.
 *
 ...
 *
 * The Run Phantom server's `agents_updated` WS event covers (1) and (2). For
 ...
 *
 * Strips the `replay:` prefix that Run Phantom puts on replay rows so callers
