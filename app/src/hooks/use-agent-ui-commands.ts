...
export function useAgentUiCommands() {
  useRunPhantomEvent("agent_ui_command", (command) => {
    ...
    if (command.type === "open_filter") {
      openRunsPage();
      window.dispatchEvent(new CustomEvent("runphantom:open-filter", { detail: command }));
      return;
    }
    if (command.type === "compose_annotation") {
      openRunsPage();
      if (command.run_id) openRun(command.run_id);
      window.dispatchEvent(new CustomEvent("runphantom:compose-annotation", { detail: command }));
    }
  });
}
